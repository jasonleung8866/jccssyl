...
# 讀取圖檔
top1 = cv2.imread("apple.png", cv2.IMREAD_UNCHANGED)
top2 = cv2.imread("orange.png", cv2.IMREAD_UNCHANGED)
top3 = cv2.imread("pear.png", cv2.IMREAD_UNCHANGED)
...
speech.setProperty('voice',voices[2].id) # 根據需要哪種語音修改 voices [ ] 方括內的數值
...
while True:
  ...
  # 改變圖片大小
  tscale = random.randint(2, 5)
  randX = random.randint(0, 300)
  randY = random.randint(0, 220)

  # 顯示及改變圖檔位置
  if (label_id == 0):
    top1b = cv2.resize(top1, (0,0), None, tscale/10, tscale/10)
    imgResult = cvzone.overlayPNG(image_src, top1b, [randX,randY])
  if (label_id == 1):
    top2b = cv2.resize(top2, (0,0), None, tscale/10, tscale/10)
    imgResult = cvzone.overlayPNG(image_src, top2b, [randX,randY])
  if (label_id == 2):
    top3b = cv2.resize(top3, (0,0), None, tscale/10, tscale/10)
    imgResult = cvzone.overlayPNG(image_src, top3b, [randX,randY])
  ...
  if cv2.waitKey(1) & 0xFF == ord('s'): # 當按下 S 鍵就讀出 Label
    speech.say(labels[label_id])
    speech.runAndWait()
